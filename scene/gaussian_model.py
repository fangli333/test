...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        #ROS-Cam:
        num_pcd = fused_point_cloud.shape[0]
        self.raw_gamma = nn.Parameter(torch.full((num_pcd, 1), 1.0).cuda().requires_grad_(True))

    def reset_gamma(self, loss):
        
        
        ###gecam
        new_raw_gamma_value = torch.clamp(loss.detach(), min=1e-10).to(self.raw_gamma.device).unsqueeze(1)
        
        
        ####rebtutall ablations
        # new_raw_gamma_value1 = torch.clamp(loss.detach(), min=1e-10).to(self.raw_gamma.device).unsqueeze(1)
        # new_raw_gamma_value = torch.ones_like(new_raw_gamma_value1)
        
        

        with torch.no_grad():
            self.raw_gamma.copy_(new_raw_gamma_value)


    def training_setup(self, training_args, cameras_extent):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
        
        self.spatial_lr_scale = cameras_extent.detach().cpu().numpy()
        ...
        l = [
            ...
            #ROS-Cam uncertainty gamma
            {'params': [self.raw_gamma], 'lr': training_args.raw_gamma_lr * self.spatial_lr_scale, "name": "raw_gamma"}
        ...
        #ROS-Cam
        self.cali_scheduler_args = get_expon_lr_func(lr_init=training_args.cali_lr_init * self.spatial_lr_scale,
                                                    lr_final=training_args.cali_lr_final * self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.cali_lr_delay_mult,
                                                    max_steps=training_args.cali_lr_max_steps)

    #ROS-Cam
    def update_learning_rate_cali(self, iteration):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                lr = self.cali_scheduler_args(iteration)
                param_group['lr'] = lr
    ...
    def prune_points(self, mask):
        ...
        #ROS-Cam:
        self.raw_gamma = optimizable_tensors["raw_gamma"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_raw_gamma):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        #ROS-Cam:
        new_raw_gamma = self.raw_gamma.squeeze()[selected_pts_mask].unsqueeze(1).repeat(N, 1)


        new_deformation_table = self._deformation_table[selected_pts_mask].repeat(N)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_deformation_table, new_raw_gamma)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        #ROS-Cam:
        new_raw_gamma = self.raw_gamma[selected_pts_mask]


        new_deformation_table = self._deformation_table[selected_pts_mask]
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_raw_gamma)
    ...
    def add_point_by_mask(self, selected_pts_mask, perturb=0):
        ...
        #ROS-Cam:
        new_raw_gamma = self.raw_gamma[selected_pts_mask][mask]
