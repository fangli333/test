...
#ROS-Cam:
def getWorld2View2_GPU(R, t, device, translate=torch.tensor([.0, .0, .0]), scale=torch.tensor(1.0)):
...
def getProjectionMatrix_GPU(znear, zfar, fovX, fovY, device):
...
#ROS-Cam
def quater2rotation(quaternion):
    q_norm = quaternion / torch.norm(quaternion)
    rotation_matrix = torch.zeros(3, 3).cuda()
    rotation_matrix[0, 0] = 1.0 - 2.0 * (q_norm[2] ** 2.0 + q_norm[3] ** 2.0)
    rotation_matrix[0, 1] = 2.0 * (q_norm[1] * q_norm[2] - q_norm[0] * q_norm[3])
    rotation_matrix[0, 2] = 2.0 * (q_norm[1] * q_norm[3] + q_norm[0] * q_norm[2])
    rotation_matrix[1, 0] = 2.0 * (q_norm[1] * q_norm[2] + q_norm[0] * q_norm[3])
    rotation_matrix[1, 1] = 1.0 - 2.0 * (q_norm[1] ** 2.0 + q_norm[3] ** 2.0)
    rotation_matrix[1, 2] = 2.0 * (q_norm[2] * q_norm[3] - q_norm[0] * q_norm[1])
    rotation_matrix[2, 0] = 2.0 * (q_norm[1] * q_norm[3] - q_norm[0] * q_norm[2])
    rotation_matrix[2, 1] = 2.0 * (q_norm[2] * q_norm[3] + q_norm[0] * q_norm[1])
    rotation_matrix[2, 2] = 1.0 - 2.0 * (q_norm[1] ** 2.0 + q_norm[2] ** 2.0)
